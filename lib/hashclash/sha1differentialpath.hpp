...
inline bf_outcome msb_bf_outcome(booleanfunction &bf, const bf_conditions &c) { return msb_bf_outcome(bf, c.first, c.second, c.third); }
inline bf_conditions
msb_bf_forwardconditions(booleanfunction &bf, bitcondition input1, bitcondition input2, bitcondition input3, bitcondition outcome) {
    if (outcome == bc_constant) {
        return bf.forwardconditions(input1, input2, input3, bc_constant);
    }
    bf_outcome o = bf.outcome(input1, input2, input3);
    if (!o.constant()) {
        return BF_simplify.forwardconditions(input1, input2, input3, bc_constant); // bf_conditions(input1, input2, input3);
    }
    if (o.plus() && o.minus()) {
        bf_conditions c = bf.forwardconditions(input1, input2, input3, bc_constant);
        if (c.third == input3) {
            std::cout << "[" << input1 << input2 << input3 << "](" << outcome << ")=>[" << c.first << c.second << c.third << "]"
                      << std::flush;
            throw std::runtime_error("msb_bf_forwardconditions: c.third == input3 ?!?");
        }
        if (c.third >= bc_next && c.third <= bc_next2n) {
            c.third =
                bitcondition(unsigned(c.third) ^ unsigned(bc_next) ^ unsigned(bc_nextn)); // bc_next <=> bc_nextn, bc_next2 <=> bc_next2n
            return c;
        }
        throw std::runtime_error("msb_bf_forwardconditions: unpredicted case ?!?");
    } else {
        return bf.forwardconditions(input1, input2, input3, outcome);
    }
}
...
inline bf_conditions
msb_bf_backwardconditions(booleanfunction &bf, bitcondition input1, bitcondition input2, bitcondition input3, bitcondition outcome) {
    if (outcome == bc_constant) {
        return bf.backwardconditions(input1, input2, input3, bc_constant);
    }
    bf_outcome o = bf.outcome(input1, input2, input3);
    if (!o.constant()) {
        return BF_simplify.backwardconditions(input1, input2, input3, bc_constant); // bf_conditions(input1, input2, input3);
    }
    if (o.plus() && o.minus()) {
        ...
        return bf.backwardconditions(input1, input2, input3, outcome);
    }
}
...
class sha1differentialpath {
  public:
    ...
    int tbegin() const { return -offset; }
    int tend() const { return int(path.size()) - offset; }
    ...
    wordconditions &operator[](int t) { return get(t); }
    ...
    void setbitcondition(int t, unsigned b, bitcondition cond) { get(t).set(b, cond); }
...
struct saveload_sha1diffpath {
    ...
    uint32 magic;
    saveload_sha1diffpath(const sha1differentialpath &path)
        : p(const_cast<sha1differentialpath &>(path)), magic(0x23580003) {}
    template <class Archive> void serialize(Archive &ar, const unsigned int file_version) {
        ar &boost::serialization::make_nvp("magic", magic);
        if (magic != 0x23580003) {
            throw std::runtime_error("serialize(): type is not sha1differentialpath");
        }
        ar &boost::serialization::make_nvp("path", p);
    }
};
template <> inline void save(const sha1differentialpath &val, archive_type artype, const path &filepath) {
    const saveload_sha1diffpath tmp(val);
    save(tmp, artype, filepath);
}
template <> inline void load(sha1differentialpath &val, archive_type artype, const path &filepath) {
...
struct saveload_vecsha1diffpath {
    ...
    uint32 magic;
    saveload_vecsha1diffpath(const std::vector<sha1differentialpath> &path)
        : p(const_cast<std::vector<sha1differentialpath> &>(path)), magic(0x23580004) {}
    template <class Archive> void serialize(Archive &ar, const unsigned int file_version) {
        ar &boost::serialization::make_nvp("magic", magic);
        if (magic == 0x23580003) {
            p.resize(1);
        ...
        if (magic != 0x23580004) {
            throw std::runtime_error("serialize(): type is not vector<sha1differentialpath>");
        }
        ar &boost::serialization::make_nvp("path", p);
    }
};
template <> inline void save(const std::vector<sha1differentialpath> &val, archive_type artype, const path &filepath) {
    const saveload_vecsha1diffpath tmp(val);
    save(tmp, artype, filepath);
}
template <> inline void load(std::vector<sha1differentialpath> &val, archive_type artype, const path &filepath) {
    saveload_vecsha1diffpath tmp(val);
    load(tmp, artype, filepath);
}
template <> inline void save_bz2(const sha1differentialpath &val, archive_type artype, const path &filepath) {
    const saveload_sha1diffpath tmp(val);
    save_bz2(tmp, artype, filepath);
}
template <> inline void load_bz2(sha1differentialpath &val, archive_type artype, const path &filepath) {
    saveload_sha1diffpath tmp(val);
    load_bz2(tmp, artype, filepath);
}
template <> inline void save_bz2(const std::vector<sha1differentialpath> &val, archive_type artype, const path &filepath) {
    const saveload_vecsha1diffpath tmp(val);
    save_bz2(tmp, artype, filepath);
}
template <> inline void load_bz2(std::vector<sha1differentialpath> &val, archive_type artype, const path &filepath) {
    saveload_vecsha1diffpath tmp(val);
    load_bz2(tmp, artype, filepath);
}
} // namespace hashclash
...
template <class Archive> void serialize(Archive &ar, hashclash::sha1differentialpath &p, const unsigned int file_version) {
