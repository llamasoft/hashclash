...
FUNC_PREFIX inline uint32 rotate_right(const uint32 x, const unsigned n) { return (x >> n) | (x << (32 - n)); }
FUNC_PREFIX inline uint32 rotate_left(const uint32 x, const unsigned n) { return (x << n) | (x >> (32 - n)); }

/**** class triple ****/
template <class Ty1, class Ty2, class Ty3> struct triple {
...
template <class Ty1, class Ty2, class Ty3> inline triple<Ty1, Ty2, Ty3> make_triple(Ty1 v1, Ty2 v2, Ty3 v3) {
    return triple<Ty1, Ty2, Ty3>(v1, v2, v3);
}

template <class Ty1, class Ty2, class Ty3> inline bool operator==(const triple<Ty1, Ty2, Ty3> &l, const triple<Ty1, Ty2, Ty3> &r) {
...
template <class Ty1, class Ty2, class Ty3> inline void swap(triple<Ty1, Ty2, Ty3> &l, triple<Ty1, Ty2, Ty3> &r) { l.swap(r); }

template <class Ty> struct sortindices {
    uint32 index;
    const Ty *ptr;
    sortindices()
        : index(~0), ptr(0) {}
    sortindices(uint32 idx, const Ty &val)
        : index(idx), ptr(&val) {}
    bool operator<(const sortindices<Ty> &r) const { return *ptr < *r.ptr; }
    bool operator==(const sortindices<Ty> &r) const { return *ptr == *r.ptr; }
};

template <class Ty1, class Ty2> inline void sortbyindex(std::vector<Ty2> &tosort, const std::vector<sortindices<Ty1>> &indices) {
...
template <class Ty1, class Ty2> void friendsort(std::vector<Ty1> &tosort, std::vector<Ty2> &tosortfriend) {
