...
struct bf_outcome {
    ...
    bool constant() const { return (c & fconstant) != 0; }
    bool plus() const { return (c & fplus) != 0; }
    bool minus() const { return (c & fminus) != 0; }
    unsigned size() const {
        unsigned w = 0;
        if (constant()) {
            ++w;
...
class booleanfunction {
  public:
    ...
    const bf_outcome &outcome(const bf_conditions &c) { return outcome(c.first, c.second, c.third); }
    ...
    uint32 F(uint32 input1, uint32 input2, uint32 input3) const { return f(input1, input2, input3); }

    const std::string &description() const { return f_description; }
