...
booleanfunction MD5_F_data(md5_ff, "MD5_FF"), MD5_G_data(md5_gg, "MD5_GG"), MD5_H_data(md5_hh, "MD5_HH"), MD5_I_data(md5_ii, "MD5_II");
...
bf_conditions booleanfunction::preferred_conditions(const bf_conditions &conds, std::vector<bf_conditions> &vec_conds) {
    ...
    for (unsigned i = 0; i < vec_conds.size(); ++i) {
        ...
        if (cond0 != bc_constant) {
            w += 16;
        }
        if (cond1 != bc_constant) {
            w += 16;
        }
        if (cond2 != bc_constant) {
            w += 16;
        }
        if (!isdirect(cond0)) {
            w += 4;
        ...
        if (isindirect2(cond0)) {
            w += 1;
        }
        if (isindirect2(cond1)) {
            w += 1;
        }
        if (isindirect2(cond2)) {
            w += 1;
        }
        if (w < bestw) {
            bestw = w;
...
booleanfunction::booleanfunction(const boost::function<uint32(uint32, uint32, uint32)> &F, const std::string &description)
    ...
    map<bitcondition, vector<bf_conditions>> outcome_to_conds_forward, outcome_to_conds_backward;
