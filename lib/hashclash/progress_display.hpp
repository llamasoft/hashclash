...
class progress_display : private boost::noncopyable {
  ...
  private:
    ...
    unsigned int _tic;
    void display_tic() {
        unsigned int tics_needed = static_cast<unsigned int>((static_cast<double>(_count) / static_cast<double>(_expected_count)) * 50.0);
        ...
        if (_tic != 49) {
            _next_tic_count = static_cast<uint64>((static_cast<double>(_tic + 1) / 50.0) * static_cast<double>(_expected_count));
