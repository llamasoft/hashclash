...
void rotate_difference(uint32 diff, int rc, std::vector<std::pair<uint32, double>> &rotateddiff) {
    ...
    if (y) {
        p1 = (bound - x) * y;
        rotateddiff.push_back(std::pair<uint32, double>(((x << rc) | y) - bound2, double(p1) * pinv));
    }

    uint32 p2 = ((bound - x) << rc) - p1;
    rotateddiff.push_back(std::pair<uint32, double>((x << rc) | y, double(p2) * pinv));
    ...
    if (x != 0) {
        ...
        if (y) {
            p3 = x * y;
            rotateddiff.push_back(std::pair<uint32, double>(((x << rc) | y) - bound2, double(p3) * pinv));
        }

        p4 = (x << rc) - p3;
        rotateddiff.push_back(std::pair<uint32, double>((x << rc) | y, double(p4) * pinv));
    }
}
...
void table_sdrs(std::vector<sdr> &result, uint32 n, unsigned maxw) {
    ...
    breaknsdr[0] = make_triple(&hashclash_sc[m0].positive, &hashclash_sc[m1 >> 11].positive, &hashclash_sc[m2 >> 21].positive);
    breaknsdr[4] = make_triple(&hashclash_sc[m0].positive, &hashclash_sc[m1 >> 11].positive, &hashclash_sc[m2 >> 21].negative);

    m2 += 0x400000;
    breakn[2] = make_triple(m0, m1, m2);
    breakn[6] = make_triple(m0, m1, m2);
    breaknsdr[2] = make_triple(&hashclash_sc[m0].positive, &hashclash_sc[m1 >> 11].negative, &hashclash_sc[m2 >> 21].positive);
    breaknsdr[6] = make_triple(&hashclash_sc[m0].positive, &hashclash_sc[m1 >> 11].negative, &hashclash_sc[m2 >> 21].negative);
    ...
    breaknsdr[1] = make_triple(&hashclash_sc[m0].negative, &hashclash_sc[m1 >> 11].positive, &hashclash_sc[m2 >> 21].positive);
    breaknsdr[5] = make_triple(&hashclash_sc[m0].negative, &hashclash_sc[m1 >> 11].positive, &hashclash_sc[m2 >> 21].negative);

    m2 += 0x400000;
    breakn[3] = make_triple(m0, m1, m2);
    breakn[7] = make_triple(m0, m1, m2);
    breaknsdr[3] = make_triple(&hashclash_sc[m0].negative, &hashclash_sc[m1 >> 11].negative, &hashclash_sc[m2 >> 21].positive);
    breaknsdr[7] = make_triple(&hashclash_sc[m0].negative, &hashclash_sc[m1 >> 11].negative, &hashclash_sc[m2 >> 21].negative);
    ...
    for (unsigned l = 0; l < breakn.size(); ++l) {
        ...
        unsigned w0 = 0;
        while (w0 < v0.size() && 0 == v0[w0].size()) {
            ++w0;
        }
        unsigned w1 = 0;
        while (w1 < v1.size() && 0 == v1[w1].size()) {
            ++w1;
        }
        unsigned w2 = 0;
        while (w2 < v2.size() && 0 == v2[w2].size()) {
            ++w2;
        }
...
void table_sdrs(std::vector<sdr> &result, uint32 n, unsigned w, bool signpos) {
    ...
    if (signpos) {
        ...
        breaknsdr[0] = make_triple(&hashclash_sc[m0].positive, &hashclash_sc[m1 >> 11].positive, &hashclash_sc[m2 >> 21].negative);
    }
    ...
    if (signpos) {
        ...
        breaknsdr[2] = make_triple(&hashclash_sc[m0].positive, &hashclash_sc[m1 >> 11].negative, &hashclash_sc[m2 >> 21].negative);
    }
    ...
    if (signpos) {
        ...
        breaknsdr[1] = make_triple(&hashclash_sc[m0].negative, &hashclash_sc[m1 >> 11].positive, &hashclash_sc[m2 >> 21].negative);
    }
    ...
    if (signpos) {
        ...
        breaknsdr[3] = make_triple(&hashclash_sc[m0].negative, &hashclash_sc[m1 >> 11].negative, &hashclash_sc[m2 >> 21].negative);
    }
    std::vector<sdr>::const_iterator cit, citend;
    for (unsigned l = 0; l < breakn.size(); ++l) {
        m0 = breakn[l].first;
    ...
    void init_sc() {
        ...
        for (temp.mask = 0; temp.mask < 0x800; ++temp.mask) {
            for (temp.sign = 0; temp.sign < 0x800; ++temp.sign) {
                if (temp.mask != (temp.sign | temp.mask)) {
                    continue;
                }
                uint32 n = temp.adddiff();
                unsigned w = temp.hw();
...
void hashclash_sdr_hpp_init() { hashclash_sdr__init here; }
