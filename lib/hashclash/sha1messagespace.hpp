...
inline void xorvec(std::vector<uint32> &srcdest, const std::vector<uint32> &toxorwith) {
    if (srcdest.size() != toxorwith.size()) {
        throw std::runtime_error("xorvec()::different sizes!");
    }
...
inline void andvec(std::vector<uint32> &srcdest, const std::vector<uint32> &toandwith) {
    if (srcdest.size() != toandwith.size()) {
        throw std::runtime_error("andvec()::different sizes!");
    }
    for (unsigned k = 0; k < srcdest.size(); ++k) {
...
inline void reduce_vector(std::vector<uint32> &vec, const std::vector<std::vector<uint32>> &basis) {
    ...
    if (vec.size() != basis[0].size()) {
        throw std::runtime_error("reduce_vector()::different sizes!");
    }
...
void derive_sha1messagespace(
    ...
    unsigned tbegin,
    unsigned tend,
    std::vector<std::vector<sdr>> &dmes,
    std::vector<double> &dmes_prob,
    double minavgprob = 0
);
...
class sha1messagespace {
  public:
    ...
    bool operator==(const sha1messagespace &r) const { return r.basis == basis && r.offset == offset; }
    bool operator!=(const sha1messagespace &r) const { return r.basis != basis || r.offset != offset; }
    void clear() {
        basis.clear();
        offset.assign(80, 0);
...
template <class Archive> void serialize(Archive &ar, hashclash::sha1messagespace &s, const unsigned int file_version) {
