...
void complement_basis(const vector<vector<uint32>> &basisin, vector<vector<uint32>> &basisout) {
    if (basisin.size() == 0) {
        throw std::runtime_error("complement_basis: trivial basis given");
    }
...
template <class iterator> class iterator_wrapper {
  public:
    typedef iterator iterator_type;
    typedef typename iterator_type::value_type value_type;

    iterator_wrapper() {}
    iterator_wrapper(const iterator_wrapper &r)
        : it(r.it) {}
    iterator_wrapper(const iterator_type &ptr)
        : it(ptr) {}

    iterator_type it;
    bool operator<(const iterator_wrapper &r) const { return **this < *r; }
    bool operator==(const iterator_wrapper &r) const { return **this < *r; }
    bool operator!=(const iterator_wrapper &r) const { return **this != *r; }
...
void derive_sha1messagespace(
    sha1messagespace &space,
    unsigned tbegin,
    unsigned tend,
    vector<vector<sdr>> &dmes,
    ...
    for (unsigned i = 0; i < dmes_sorted.size(); ++i) {
        if (i > 0 && dmes_sorted[i] == dmes_sorted[i - 1]) {
            continue;
        }
        unsigned j = i + 1;
    ...
    */
    ...
    for (unsigned i = 1; i < dmes.size(); ++i) {
        for (unsigned t = 0; t < 80; ++t) {
            if (dmes[i][t].mask != bitmask[t]) {
                throw std::runtime_error("derive_sha1messagespace(): dmes not sane!");
            }
        }
    }
    ...
    while (true) {
            ...
            if (!dmes_inuse[i]) {
                ...
                for (unsigned j = 0; j < dmes.size() && i_is_ok; ++j) {
                    if (dmes_inuse[j]) {
                        ...
                        vector<dme_wrapper>::const_iterator cit =
                            lower_bound(dmes_sorted.begin(), dmes_sorted.end(), dme_wrapper(tmpdme.begin()));
        ...
        if (newavgprob >= 0 && newavgprob >= minavgprob) {
            ++freedomgained;
            cout << "added new basis vector: avgprob(" << log(avgprob) / log(2.0) << "=>" << log(newavgprob) / log(2.0)
                 << ") freedom gained(" << freedomgained << ")" << endl;
            for (int t = tbegin; t < tend; ++t) {
            ...
            for (unsigned j = 0; j < dmes.size(); ++j) {
                if (dmes_inuse[j]) {
                    ...
                    pair<vector<dme_wrapper>::const_iterator, vector<dme_wrapper>::const_iterator> citpair =
                        equal_range(dmes_sorted.begin(), dmes_sorted.end(), dme_wrapper(tmpdme.begin()));
                    vector<dme_wrapper>::const_iterator cit =
                        citpair.first; // lower_bound( dmes_sorted.begin(), dmes_sorted.end(), dme_wrapper(tmpdme.begin()) );
            ...
            break;
            cout << "no new basis vector: avgprob(" << log(avgprob) / log(2.0) << ") bestnewavgprob(" << log(newavgprob) / log(2.0)
                 << ") freedom gained(" << freedomgained << ")" << endl;
        }
    }
