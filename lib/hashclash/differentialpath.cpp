...
double check_rotation(uint32 dR, uint32 dT, unsigned n, const wordconditions &Qt, const wordconditions &Qtp1, unsigned loopcount) {
    ...
    uint32 prev = Qtp1.prev() | Qt.next(), prevn = Qtp1.prevn() | Qt.nextn();
...
unsigned totaltunnelstrength(const differentialpath &path) {
    ...
    for (unsigned b = 0; b < 32; ++b) {
        ...
        if (q8b == bc_constant && q9b != bc_prev && q9b != bc_prevn && q12b == bc_constant && q13b != bc_prev && q13b != bc_prevn &&
            (q10b == bc_constant || q10b == bc_minus || q10b == bc_one)) {
            // Q8Q12m15 tunnel
            ++totalstrength;
            // Q9m10 tunnel is only other possible
            if (q9b == bc_constant && (q10b == bc_constant || q10b == bc_one || q10b == bc_minus) &&
    ...
    totalstrength += (hw(Q0Q1eq & Q1free & ~path[2].mask()) >> 1) + hw(Q0Q1eq & Q1free & path[2].prev());
...
void cleanup(differentialpath &path) {
    ...
    for (int t = path.tbegin() + 3; t <= path.tend() - 2; ++t) {
        ...
        for (unsigned b = 0; b < 32; ++b) {
            ...
            if (outcome.size() != 1) {
                throw std::runtime_error("hashclash::cleanup(differentialpath&): path is ambiguous!");
            }
...
struct solutions_less : public std::binary_function<soltriple, soltriple, bool> {
    bool operator()(const soltriple &_Left, const soltriple &_Right) const { return _Left.third < _Right.third; }
};
...
bool isstronger(const soltriple &lh, const soltriple &rh) {
    ...
    // rh=1 => lh=1
    if (rhtt0 != (rhtt0 & lhtt0)) {
        return false;
    }
    if (rhtt1 != (rhtt1 & lhtt1)) {
        return false;
    }

    // rh=^ => lh=0,1,^
    if (rhttp != (rhttp & (lhtt0 | lhtt1 | lhttp))) {
        return false;
    }
    // rh=! => lh=0,1,!
    if (rhttpn != (rhttpn & (lhtt0 | lhtt1 | lhttpn))) {
        return false;
    }
...
bool errorinrotation32(
    const wordconditions &Qt, const wordconditions &Qtp1, uint32 dT, uint32 dR, unsigned rc, uint32 loopcount = (1 << 12)
) {
...
void findsolutions32(
    const wordconditions &Qt,
    const wordconditions &Qtp1,
    uint32 dT,
    uint32 dR,
    unsigned rc,
    ...
    if (bc == newpath(t, b) || bc == bc_constant) {
        ...
        if (bc == bc_one || bc == bc_zero) {
            ...
            while (newpath(k, b) != bc_constant && newpath(k, b) != bck) {
                ...
                if (newpath(k, b) == bc_one || newpath(k, b) == bc_zero || newpath(k, b) == bc_plus || newpath(k, b) == bc_minus) {
                    cerr << "t=" << t << ",b=" << b << ",bc=" << bc << ",path(t,b)=" << newpath(t, b) << endl;
