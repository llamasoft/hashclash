...
template <class value_type> void save_bz2(const value_type &val, const std::string &filepath, archive_type artype) {
    switch (artype) {
    case binary_archive:
...
template <class value_type> void save_bz2(const value_type &val, archive_type artype, const path &filepath) {
    std::ofstream ofs(filepath.string().c_str(), std::ios::binary);
    if (!ofs) {
        throw std::runtime_error("save_bz2(): could not open file!");
    }
    ...
    if (!ofs) {
        throw std::runtime_error("save_bz2(): write error!");
    }
}

template <class value_type> void load_bz2(value_type &val, const std::string &filepath, archive_type artype) {
    switch (artype) {
    case binary_archive:
...
template <class value_type> void load_bz2(value_type &val, archive_type artype, const path &filepath) {
    std::ifstream ifs(filepath.string().c_str(), std::ios::binary);
    // fs::ifstream ifs(filepath, std::ios::binary);
    if (!ifs) {
        throw std::runtime_error("load_bz2(): could not open file!:" + filepath.string());
    }
    ...
    if (!ifs) {
        throw std::runtime_error("load_bz2(): read error!");
    }
}

/* specialization types to distinguish between differentialpath & vector<differentialpath> */
template <> inline void save_bz2(const differentialpath &val, archive_type artype, const path &filepath) {
    const saveload_diffpath tmp(val);
    save_bz2(tmp, artype, filepath);
}
template <> inline void load_bz2(differentialpath &val, archive_type artype, const path &filepath) {
    saveload_diffpath tmp(val);
    load_bz2(tmp, artype, filepath);
}
template <> inline void save_bz2(const std::vector<differentialpath> &val, archive_type artype, const path &filepath) {
    const saveload_vecdiffpath tmp(val);
    save_bz2(tmp, artype, filepath);
}
template <> inline void load_bz2(std::vector<differentialpath> &val, archive_type artype, const path &filepath) {
