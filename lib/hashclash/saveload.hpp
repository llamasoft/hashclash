...
enum archive_type { binary_archive = 0, xml_archive, text_archive };

template <class value_type> void save(const value_type &val, const std::string &filepath, archive_type artype) {
    switch (artype) {
    case binary_archive:
...
template <class value_type> void save(const value_type &val, archive_type artype, const path &filepath) {
    switch (artype) {
    case binary_archive: {
        std::ofstream ofs(filepath.string().c_str(), std::ios::binary);
        if (!ofs) {
            throw std::runtime_error("save(): could not open file!");
        }
...
template <class value_type> void load(value_type &val, const std::string &filepath, archive_type artype) {
    switch (artype) {
    case binary_archive:
...
template <class value_type> void load(value_type &val, archive_type artype, const path &filepath) {
    switch (artype) {
    case binary_archive: {
        ...
        if (!ifs) {
            throw std::runtime_error("load(): read error!");
        }
    } break;
...
struct saveload_diffpath {
    differentialpath &p;
    uint32 magic;
    saveload_diffpath(const differentialpath &path)
        : p(const_cast<differentialpath &>(path)), magic(0x23580001) {}
    template <class Archive> void serialize(Archive &ar, const unsigned int file_version) {
        ar &boost::serialization::make_nvp("magic", magic);
        if (magic != 0x23580001) {
            throw std::runtime_error("serialize(): type is not differentialpath");
        }
        ar &boost::serialization::make_nvp("path", p);
    }
};
template <> inline void save(const differentialpath &val, archive_type artype, const path &filepath) {
    const saveload_diffpath tmp(val);
    save(tmp, artype, filepath);
}
template <> inline void load(differentialpath &val, archive_type artype, const path &filepath) {
...
struct saveload_vecdiffpath {
    ...
    uint32 magic;
    saveload_vecdiffpath(const std::vector<differentialpath> &path)
        : p(const_cast<std::vector<differentialpath> &>(path)), magic(0x23580002) {}
    template <class Archive> void serialize(Archive &ar, const unsigned int file_version) {
        ar &boost::serialization::make_nvp("magic", magic);
        if (magic != 0x23580002) {
            throw std::runtime_error("serialize(): type is not vector<differentialpath>");
        }
        ar &boost::serialization::make_nvp("path", p);
    }
};
template <> inline void save(const std::vector<differentialpath> &val, archive_type artype, const path &filepath) {
    const saveload_vecdiffpath tmp(val);
    save(tmp, artype, filepath);
}
template <> inline void load(std::vector<differentialpath> &val, archive_type artype, const path &filepath) {
