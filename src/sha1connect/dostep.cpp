...
struct dostep_thread {
    ...
    void operator()() {
        try {
            ...
            cerr << "Worker thread: caught unknown exception:" << endl;
        }
    }
};
void dostep_threaded(
    vector<sha1differentialpath> &inlow, vector<sha1differentialpath> &inhigh, path_container &out, bool randomize = true
) {
    mut.lock();
    dostep_index = 0;
    ...
    if (pathsredo.size() > 0) {
        ...
        for (unsigned k = 0; k < pathsinlow.size(); ++k, ++pd) {
            ...
            for (unsigned j = 0; j < pathsredo.size(); ++j) {
                ...
                //				if (pathsredo[j].tbegin() > pathsinlow[k].tbegin() || pathsredo[j].tend() <
                //pathsinlow[k].tend()) 					continue;
                bool ok = true;
        ...
        for (unsigned k = 0; k < pathsinhigh.size(); ++k, ++pd) {
            ...
            for (unsigned j = 0; j < pathsredo.size(); ++j) {
                ...
                //				if (pathsredo[j].tbegin() > pathsinhigh[k].tbegin() || pathsredo[j].tend() <
                //pathsinhigh[k].tend()) 					throw std::runtime_error("blaaat"); 					continue;
                bool ok = true;
