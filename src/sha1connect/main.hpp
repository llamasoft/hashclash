...
struct connect_bitdata {
    ...
    inline bool operator==(const connect_bitdata &r) const {
        ...
        return dQtm1 == r.dQtm1 && dQt == r.dQt && dQtp1 == r.dQtp1 && dFt == r.dFt && dFtp1 == r.dFtp1 && dFtp2 == r.dFtp2 &&
               dFtp3 == r.dFtp3 && dFtp4 == r.dFtp4;
    }
    inline bool operator!=(const connect_bitdata &r) const { return !(*this == r); }
    inline bool operator<(const connect_bitdata &r) const {
        if (dQtm1 < r.dQtm1) {
            return true;
        }
        ...
        for (unsigned i = 0; i < 2; ++i) {
            if (rqtm2[i] < r.rqtm2[i]) {
                return true;
            }
            if (rqtm2[i] > r.rqtm2[i]) {
                return false;
            }
            if (rqtm1[i] < r.rqtm1[i]) {
                return true;
            }
            if (rqtm1[i] > r.rqtm1[i]) {
                return false;
            ...
            if (rqtp1[i] < r.rqtp1[i]) {
                return true;
            }
            if (rqtp1[i] > r.rqtp1[i]) {
                return false;
            }
            if (fqtm1[i] < r.fqtm1[i]) {
                return true;
            }
            if (fqtm1[i] > r.fqtm1[i]) {
                return false;
            ...
            if (fqtp1[i] < r.fqtp1[i]) {
                return true;
            }
            if (fqtp1[i] > r.fqtp1[i]) {
                return false;
            }
            if (fqtp2[i] < r.fqtp2[i]) {
                return true;
            }
            if (fqtp2[i] > r.fqtp2[i]) {
                return false;
...
struct sha1_connect_thread {
    ...
    void connectbits_01234(
        const pair<connect_bitdata, unsigned> &in,
        ...
        vector<connect_bitdata> &dataend,
        vector<unsigned> &mincond
    );
    template <bool storenewconds>
    void connectbits_1234(
        const pair<connect_bitdata, unsigned> &in,
        map<connect_bitdata, unsigned> &out,
        unsigned b,
        const sha1differentialpath &lower,
        ...
        for (int k = tunnelconditions.tbegin(); k < tunnelconditions.tend() && !bad; ++k) {
            if (k >= fullpath.tbegin() && k < fullpath.tend()) {
                for (unsigned b = 0; b < 32 && !bad; ++b) {
                    ...
                    if (bcpath == bc_constant && bc != bc_prev && bc != bc_prevn) {
                        continue;
                    }
                    if (bcpath == bc_plus) {
                        bcpath = bc_zero;
                    } else if (bcpath == bc_minus) {
                        bcpath = bc_one;
                    }
                    switch (bc) {
                    case bc_constant:
                    ...
                    case bc_zero:
                        ...
                        if (bcpath == bc_one || bcpath == bc_zero) {
                            bad = true;
                            ++badcounts[4 + k][b];
                            break;
                        }
                        if (bcpath == bc_plus || bcpath == bc_minus) {
                            bad = true;
                            ++badcounts[4 + k][b];
                            break;
                        }
                    ...
                    case bc_prev: {
                        ...
                        //                                                if (bcpath == bc_plus || bcpath == bc_minus || bcpathm1 == bc_plus
                        //                                                || bcpathm1 == bc_minus) return;
                        if (bcpathm1 == bc_constant) {
                            break;
                        }
                        if (bcpathm1 == bc_one && bcpath == bc_zero) {
                            bad = true;
                            ++badcounts[4 + k][b];
                            break;
                        }
                        if (bcpathm1 == bc_zero && bcpath == bc_one) {
                            bad = true;
                            ++badcounts[4 + k][b];
                            break;
                        }
                    ...
                    case bc_prevn: {
                        ...
                        if (bcpathm1 == bc_one && bcpath == bc_one) {
                            bad = true;
                            ++badcounts[4 + k][b];
                            break;
                        }
                        if (bcpathm1 == bc_zero && bcpath == bc_zero) {
                            bad = true;
                            ++badcounts[4 + k][b];
                            break;
                        }
                    ...
                    if (bad && hw(uint32(badcounts[4 + k][b])) == 1) {
                        show = true;
                    }
                }
            }
...
struct diffpathupper_less : public std::binary_function<sha1differentialpath, sha1differentialpath, bool> {
    bool operator()(const sha1differentialpath &_Left, const sha1differentialpath &_Right) const {
        ...
        unsigned b = 0;
        while (b < 32 + 8) {
            if (b < 32 && (LdFt ^ RdFt) & (1 << b)) {
                break;
            }
            if (b >= 2 && b - 2 < 32 && ((LdFtp1 ^ RdFtp1) & (1 << (b - 2)))) {
                break;
            }
            if (b >= 4 && b - 4 < 32 && ((LdQtp1 ^ RdQtp1) & (1 << (b - 4)))) {
                break;
            }
            if (b >= 4 && b - 4 < 32 && ((LdFtp2 ^ RdFtp2) & (1 << (b - 4)))) {
                break;
            }
            if (b >= 6 && b - 6 < 32 && _Left(t + 2, b - 6) != _Right(t + 2, b - 6)) {
                break;
            }
            if (b >= 6 && b - 6 < 32 && ((LdFtp3 ^ RdFtp3) & (1 << (b - 6)))) {
                break;
            }
            if (b >= 8 && b - 8 < 32 && _Left(t + 3, b - 8) != _Right(t + 3, b - 8)) {
                break;
            }
            if (b >= 8 && b - 8 < 32 && ((LdFtp4 ^ RdFtp4) & (1 << (b - 8)))) {
                break;
            }
            ++b;
        }
        ...
        if (b >= 6 && b - 6 < 32) {
            if (_Left(t + 2, b - 6) < _Right(t + 2, b - 6)) {
                return true;
            }
            if (_Left(t + 2, b - 6) > _Right(t + 2, b - 6)) {
                return false;
        ...
        if (b >= 8 && b - 8 < 32) {
            if (_Left(t + 2, b - 8) < _Right(t + 2, b - 8)) {
                return true;
            }
            if (_Left(t + 2, b - 8) > _Right(t + 2, b - 8)) {
                return false;
...
struct diffpathlower_less : public std::binary_function<sha1differentialpath, sha1differentialpath, bool> {
    bool operator()(const sha1differentialpath &_Left, const sha1differentialpath &_Right) const {
        if (_Left.tend() < _Right.tend() || _Left.tbegin() > _Right.tbegin()) {
            return true;
        }
        ...
        while (b < 32 + 8) {
            if (b < 32) {
                if (_Left(t - 3, (b + 2) & 31) != _Right(t - 3, (b + 2) & 31)) {
                    break;
                }
                if (_Left(t - 2, (b + 2) & 31) != _Right(t - 2, (b + 2) & 31)) {
                    break;
                }
                if ((LdFt ^ RdFt) & (1 << b)) {
                    break;
                }
                if ((LdQtm1 ^ RdQtm1) & (1 << b)) {
                    break;
                }
            }
            if (b >= 2 && b - 2 < 32) {
                if ((LdFtp1 ^ RdFtp1) & (1 << (b - 2))) {
                    break;
                }
                if ((LdQt ^ RdQt) & (1 << (b - 2))) {
                    break;
...
struct diffpathlower_less_weight : public std::binary_function<sha1differentialpath, sha1differentialpath, bool> {
    bool operator()(const sha1differentialpath &_Left, const sha1differentialpath &_Right) const {
        if (_Left.tend() < _Right.tend() || _Left.tbegin() > _Right.tbegin()) {
            return true;
        }
