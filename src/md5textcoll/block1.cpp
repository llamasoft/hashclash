...
void textcoll_solver_t::start_block1() {
    ...
    std::sort(Q7Q24states.begin(), Q7Q24states.end(), [this](const halfstate_t &l, const halfstate_t &r) {
...
void textcoll_solver_t::completeQ7Q24(const halfstate_t &Q7Q24state) {
    ...
    */
    ...
    for (auto m4 : MA.shuffled_word_range(4)) {
        ...
        run_workload(threads, [this, &m13Q10good, &pdQ7810, &m13readptr, &m13writeptr, &S, &Q7810m1213](size_t ji, size_t jn) {
            fullstate_t myS = S;
            ...
            while (!stop) {
                ...
                for (uint32_t m12 : m12range) {
                    ...
                    tmp.insert(std::make_pair(
                        std::array<uint32_t, 3>({myS.Qt(7), myS.Qt(8), myS.Qt(10)}), std::array<uint32_t, 2>({myS.m[12], myS.m[13]})
                    ));
                }
                ...
                // write results in tmp to Q7810m1213
                {
                    ...
                    for (auto &kv : tmp) {
                        ...
                        if (itf.second && hammingweight(uint64_t(Q7810m1213.size())) == 1) {
                            std::cout << Q7810m1213.size() << " " << std::flush;
                        }
                    }
