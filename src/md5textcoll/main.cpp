...
int main(int argc, char **argv) {
    ...
    cout << "MD5 differential path textcollision solver\n"
    ...
    try {
        ...
        po::options_description cmds("Allowed commands"), desc("Allowed options"),
            msg("Define message differences (as +bitnr and -bitnr, bitnr=1-32)"), bba("Byte-specific alphabets"), all("Allowed options");

        cmds.add_options()("help,h", "Show options\n")("prepare,p", "Prepare 1st near-collision block search\n")("firstblock,f", "Find 1st near-collision block\n")(
        ...
        if (vm.count("help") || 1 != vm.count("prepare") + vm.count("firstblock") + vm.count("secondblock")) {
            cout << cmds << desc << endl;
            return 0;
        }
        ...
        if (vm.count("secondblock")) {
            ...
            cout << dec << endl;
            cout << "dIHV   = " << naf(ihv2[0] - ihv1[0]) << naf(ihv2[1] - ihv1[1]) << naf(ihv2[2] - ihv1[2]) << naf(ihv2[3] - ihv1[3])
                 << std::endl;
        }
