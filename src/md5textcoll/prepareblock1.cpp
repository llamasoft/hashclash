...
void textcoll_solver_t::prepare_block1() {
    ...
    for (int wt : {0, 1, 4, 5, 6, 10, 11, 12, 13, 14, 15}) {
        for (unsigned b = 0; b < 4; ++b) {
            ...
            std::sort(
                charcountsorted.begin(),
                charcountsorted.end(),
                [](const std::pair<char, size_t> &l, const std::pair<char, size_t> &r) { return l.second > r.second; }
            );
...
void textcoll_solver_t::generateQ13Q17(vec_halfstate_t &out, uint64_t N) {
    ...
    uint64_t attempts = 0;
    run_workload(threads, [this, &job_range, &attempts, &out, &pd](size_t ji, size_t jn) {
        ...
        for (auto k = begin; k < end;) {
            ...
            if (!checkrotationQtQtp1(13, S)) {
                continue;
            }
            if (!checkrotationQtQtp1(14, S)) {
                continue;
            }
            if (!checkrotationQtQtp1(15, S)) {
                continue;
            }
    ...
    std::cout << "Generate Q13-Q17,m1: attempts: 2^" << log(double(attempts)) / log(2.0) << " success rate: 2^"
...
void textcoll_solver_t::extend_step_fw(int t, vec_halfstate_t &out, const vec_halfstate_t &in, uint64_t N) {
    ...
    uint64_t attempts = 0;
    run_workload(threads, [this, &job_range, &attempts, &out, &in, &pd, t, wt](size_t ji, size_t jn) {
        auto begin = job_range[ji].first, end = job_range[ji].second;
        uint64_t myattempts = 0;
        localxrng rng;
    ...
    std::cout << "Step " << t << ": attempts: 2^" << log(double(attempts)) / log(2.0) << ", success rate: 2^"
              << log(double(N) / double(attempts)) / log(2.0) << std::endl;
}
...
void textcoll_solver_t::extend_step_m11(vec_halfstate_t &out, const vec_halfstate_t &in, uint64_t N) {
    ...
    uint64_t attempts = 0;
    run_workload(threads, [this, &job_range, &attempts, &out, &in, &pd](size_t ji, size_t jn) {
        auto begin = job_range[ji].first, end = job_range[ji].second;
        uint64_t myattempts = 0;
        localxrng rng;
    ...
    std::cout << "Step m11: attempts: 2^" << log(double(attempts)) / log(2.0) << ", success rate: 2^"
              << log(double(N) / double(attempts)) / log(2.0) << std::endl;
}
...
void textcoll_solver_t::extend_step_m10(vec_halfstate_t &out, const vec_halfstate_t &in, uint64_t N) {
    ...
    std::cout << "Step m10: attempts: 2^" << log(double(attempts)) / log(2.0) << ", success rate: 2^"
              << log(double(N) / double(attempts)) / log(2.0) << std::endl;
}
