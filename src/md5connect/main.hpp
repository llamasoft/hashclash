...
struct md5_connect_thread {
    md5_connect_thread()
        : sw(true), countb(33, 0), countbaborted(33, 0), countbdepth(33, 0) {}
    ...
    unsigned
    md5_connect_bits(const vector<differentialpath> &lowers, unsigned index, const differentialpath &upper, path_container &container);
    ...
    inline unsigned binary_search_lower_paths(const vector<differentialpath> &lowerpaths, unsigned i, unsigned b) {
    ...
    void push_back(const differentialpath &fullpath) {
        ...
        if (tuncompl == bestmaxcomp && tunnel == bestmaxtunnel && cond <= bestpathcond + 4) {
            ...
            if (hw(uint32(bestpaths.size())) == 1) {
                ...
                save_gz(
                    bestpaths,
                    workdir + "/bestpaths_t" + boost::lexical_cast<string>(tunnel) + "_c" + boost::lexical_cast<string>(cond),
                    binary_archive
                );
        ...
        cout << "Best path: totcompl=" << bestmaxcomp << " tottunnel=" << bestmaxtunnel << ", totcond=" << bestpathcond << ", p=" << p
             << endl;
        save_gz(
            pathback,
            workdir + "/bestpath_t" + boost::lexical_cast<string>(tunnel) + "_c" + boost::lexical_cast<string>(cond),
            binary_archive
        );
        save_gz(
            bestpaths,
        ...
        try {
            boost::filesystem::rename(workdir + "/bestpath.bin.gz", workdir + "/bestpath_old.bin.gz");
        } catch (...) {
        }
        try {
            boost::filesystem::rename(workdir + "/bestpaths.bin.gz", workdir + "/bestpaths_old.bin.gz");
        } catch (...) {
        }
        try {
            boost::filesystem::rename(workdir + "/bestpath_new.bin.gz", workdir + "/bestpath.bin.gz");
        } catch (...) {
        }
        try {
            boost::filesystem::rename(workdir + "/bestpaths_new.bin.gz", workdir + "/bestpaths.bin.gz");
        } catch (...) {
        }
        mut.unlock();
    }
...
struct diffpathlower_less : public std::binary_function<differentialpath, differentialpath, bool> {
...
struct diffpathupper_less : public std::binary_function<differentialpath, differentialpath, bool> {
    bool operator()(const differentialpath &_Left, const differentialpath &_Right) const {
        if (_Left.tend() < _Right.tend() || _Left.tbegin() > _Right.tbegin()) {
            return true;
        }
