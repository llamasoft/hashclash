...
void md5_connect_thread::connectbits(
    const connect_bitdata &in,
    vector<connect_bitdata> &out,
    unsigned b,
    const differentialpath &lower,
    const differentialpath &upper,
                    ...
                    for (unsigned bfo2index = 0; bfo2index < bfo2.size(); ++bfo2index) {
                        ...
                        for (unsigned bfo3index = 0; bfo3index < bfo3.size(); ++bfo3index) {
                            ...
                            if (newconds) {
                                ...
                                newcond.set(bfc0.third, bfc1.third, bfc2.third, bfc3.third, bfc3.second, bfc3.first);
...
void md5_connect_thread::connectbits2(const connect_bitdata &in, unsigned b, const differentialpath &lower, const differentialpath &upper) {
    //	static bf_outcome bfo0, bfo1, bfo2, bfo3;
    //	static bf_conditions bfc0, bfc1, bfc2, bfc3;
    ...
    for (unsigned dqt = 0; dqt <= dQtbit; ++dqt) {
        ...
        bfo0 = Ft->outcome(Qt, Qtm1, Qtm2);
        for (unsigned bfo0index = 0; bfo0index < bfo0.size(); ++bfo0index) {
            if ((in.dFt - bfo0(bfo0index, b)) & (1 << b)) {
                continue;
            }
            bfc0 = Ft->forwardconditions(Qt, Qtm1, Qtm2, bfo0[bfo0index]);
            ...
            for (unsigned dqtp1 = 0; dqtp1 <= dQtp1bit; ++dqtp1) {
                ...
                bfo1 = Ftp1->outcome(Qtp1, bfc0.first, bfc0.second);
                for (unsigned bfo1index = 0; bfo1index < bfo1.size(); ++bfo1index) {
                    if ((in.dFtp1 - bfo1(bfo1index, b)) & (1 << b)) {
                        continue;
                    }
                    bfc1 = Ftp1->forwardconditions(Qtp1, bfc0.first, bfc0.second, bfo1[bfo1index]);
                    ...
                    bfo2 = Ftp2->outcome(Qtp2, bfc1.first, bfc1.second);
                    for (unsigned bfo2index = 0; bfo2index < bfo2.size(); ++bfo2index) {
                        if ((in.dFtp2 - bfo2(bfo2index, b)) & (1 << b)) {
                            continue;
                        }
...
unsigned md5_connect_thread::md5_connect_bits(
    const vector<differentialpath> &lowers, unsigned index, const differentialpath &upper, path_container &container
) {
    ...
    for (unsigned b = 0; b < 32; ++b) {
        ...
        bitdataresults[b + 1].erase(unique(bitdataresults[b + 1].begin(), bitdataresults[b + 1].end()), bitdataresults[b + 1].end());
    }
    ...
    if (container.Qcondstart > t + 3) {
        ...
        for (unsigned b = 0; b < 32; ++b) {
            ...
            for (unsigned i = 0; i < bitdatanewcond[b].size(); ++i) {
                ...
                unsigned bcc = bc;
                if (bitdatanewcond[b][i][0] != bc_constant) {
                    ++bcc;
                }
                if (bitdatanewcond[b][i][1] != bc_constant) {
                    ++bcc;
                }
                if (bitdatanewcond[b][i][2] != bc_constant) {
                    ++bcc;
                }
                if (bitdatanewcond[b][i][3] != bc_constant) {
                    ++bcc;
                }
                if (bitdatanewcond[b][i][4] != bc_constant) {
                    ++bcc;
                }
                if (bitdatanewcond[b][i][5] != bc_constant) {
                    ++bcc;
                }
        ...
        for (int b = 0; b < 31; ++b) {
            for (unsigned i = 0; i < bitdatanewcond[b].size(); ++i) {
                ...
                pair<vector<connect_bitdata>::const_iterator, vector<connect_bitdata>::const_iterator> bounds =
                    equal_range(bitdatastart[b + 1].begin(), bitdatastart[b + 1].end(), bitdataend[b][i]);
                for (; bounds.first != bounds.second; ++bounds.first) {
                    unsigned j = bounds.first - bitdatastart[b + 1].begin();
                    if (tunnelstrength[b + 1][j] + cts >= cumtunnelstrength[b + 1][j]) {
                        if (tunnelstrength[b + 1][j] + cts > cumtunnelstrength[b + 1][j] ||
                            colbitconditions[b + 1][j] + cumbitconditions[b][i] < cumbitconditions[b + 1][j]) {
        ...
        if (maxcompl < container.bestmaxcomp) {
            return 32;
        }
        if (maxtunnelstrength < container.bestmaxtunnel) {
            return 32;
        }
        if (minconditions > container.bestpathcond) {
            return 32;
        }
    }
#endif
    ...
    while (bit <= 31) {
        if (bindex[bit] < bitdataend[bit].size()) {
            ...
            if (bit != 0) {
...
#if 1
                        if (cumtunnelstrength[bit][k] + ts < container.bestmaxtunnel) {
                            continue;
                        }
                        if (cumtunnelstrength[bit][k] + ts == container.bestmaxtunnel &&
                            cumbitconditions[bit][k] + bc > container.bestpathcond) {
                            continue;
                        }
#endif
                        bindex[bit] = k;
                ...
                for (unsigned k = 0; k < bitdataend[bit].size(); ++k) {
                    if (bitdataend[bit][k] == bitdatastart[bit + 1][bindex[bit + 1]]) { // we have a full path !!!
                        bindex[0] = k;
#if 1
                        if (cumtunnelstrength[bit][k] + ts < container.bestmaxtunnel) {
                            continue;
                ...
                if (bit < 31) {
                    while (bindex[bit] < bitdataend[bit].size() &&
                           (((bitdataend[bit][bindex[bit]]) != (bitdatastart[bit + 1][bindex[bit + 1]]))
                            ...
                            (cumtunnelstrength[bit][bindex[bit]] + ts == container.bestmaxtunnel &&
                             cumbitconditions[bit][bindex[bit]] + bc > container.bestpathcond)
#endif
                           ))
                        ++bindex[bit];
                } else {
...
void md5_connect_thread::md5_connect(
    const vector<differentialpath> &lowerpaths, const differentialpath &upperpath, path_container &container
) {
    ...
    if (container.showstats && sw.time() > 60) {
        cout << endl << count << "\t" << countall << "\t" << double(countall) / double(count) << endl;
        for (unsigned b = 0; b <= 32; ++b) {
            cout << b << ":\t" << countb[b] << "\t" << countbaborted[b] << "\t" << double(countbaborted[b]) / double(countb[b]) << "\t"
    ...
    if (0 != newpath.path.size()) {
        while (bequal < 32 && newpath[t + 1][bequal] == upperpath[t + 1][bequal] && newpath[t + 2][bequal] == upperpath[t + 2][bequal] &&
    ...
    for (unsigned i = 0; i < lowerpaths.size(); ++i) {
        ...
        if (isgood[i] != 0) {
            if (isgood[i] <= bequal) {
                if (0 != ((dFti ^ dFt[i]) << (32 - isgood[i])) || 0 != ((dFtp1i ^ dFtp1[i]) << (32 - isgood[i])) ||
                    0 != ((dFtp2i ^ dFtp2[i]) << (32 - isgood[i])) || 0 != ((dFtp3i ^ dFtp3[i]) << (32 - isgood[i]))) {
                    isgood[i] = 0;
                }
            } else {
                isgood[i] = 0;
    ...
    for (unsigned i = 0; i < lowerpaths.size(); ++i) {
        ...
        if (lowerpathsmaxtunnel[i] + uppercompl <= container.bestmaxcomp) {
            continue;
        }
        ++count;
        ...
        if (b < 32) {
            ...
            for (; j < jmax; ++j) {
                ...
                if (dFti == (dFt[j] & maskt) && dFtp1i == (dFtp1[j] & masktp1) && dFtp2i == (dFtp2[j] & masktp2) &&
