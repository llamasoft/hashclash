...
template <class _Ty> string array_to_cppstring(const _Ty vals[], unsigned cnt) {
    if (cnt == 0) {
        return string("{}");
    }
...
template <class _Ty> string vector_to_cppstring(const vector<_Ty> &vals) { return array_to_cppstring(&vals[0], vals.size()); }
...
string mask2bit_to_string(const string &variable, const uint32 mask, const unsigned bit) {
    ...
    if (hw(mask) == 1) {
        for (unsigned b = 0; b < 32; ++b) {
            if (mask & (1 << b)) {
                if (b == bit) {
                    return "(" + variable + "&" + LCSTRING(1 << b) + ")";
                }
                if (b < bit) {
                    return "((" + variable + "&" + LCSTRING(1 << b) + ")<<" + LCSTRING(bit - b) + ")";
                }
                if (b > bit) {
                    return "((" + variable + "&" + LCSTRING(1 << b) + ")>>" + LCSTRING(b - bit) + ")";
                }
            }
        }
    }
...
string generate_program_step(unsigned t) {
    ...
    of << "\nbool step_" << boost::lexical_cast<string>(t)
       ...
       << "static bool firsttime = true;"
          "\n	"
       << (t >= 16 ? "//" : "") << "if (firsttime) { cout << \"t" << t
       << " \" << endl; firsttime = false; }"
          "\n	const unsigned t = "
       << t
       << ";"
          "\n	"
       ...
       << "ok = false;"
          ...
          "\n	const uint32 metmask = "
       << metmask << ";";
    if (t == 0) {
        // alternative beginning for t = 0
        of << "\n	{"
              "\n		const uint32 metset1 = "
           << metset1 << ";";
    } else {
        of << "\n	uint32* Qtcheckptr = Qtp1valsvec[20];"
              ...
              "\n		const uint32 metset1 = metset1precomp["
           << t - 1 << "][" << t << "] ^ "
           << optimized_mecond_to_string("m[" + boost::lexical_cast<string>(t - 1) + "]", met_dependency_metm1) << ";";
    }
    if (hw(Qvaluemask_adj[offset + t + 1]) >= hw(metmask)) {
        ...
        if (hw(Qtp1mask & (~metmask)) <= MAX_LOOP_SIZE || (metmask & ~Qtp1mask) == 0) {
            ...
            of << "\n		const uint32 Qtp1val = Qvalue[offset+t+1] ^ (Qprev[offset+t+1]&Q[offset+t]) ^ ((~metset1) & " << metmaskfix
               ...
               << ");"
                  ...
                  "\n		uint32 cur = 0; const uint32 curmask = "
               << curmask
               << ";"
               ...
               << metmasknofix
               << ")) break;"
                  "\n			uint32 metxorfix = (m[t]^metset1) & "
               << metmaskfix
               << ";"
                  "\n			*(Qtp1valsptr++) = Q[offset+t+1]^metxorfix;"
                  "\n		} while (Qtp1cur != 0);"
                  "\n		// loop_data_vec[40+t].add(Qtp1valsptr-ptrbu, 1<<hw(uint32("
               << Qtp1mask
               << "^curmask)));"
                  "\n		} while (cur != 0);"
                  "\n		time_avg[t] += Qtp1valsptr-Qtp1valsvec[t];"
                  "\n		loop_data_vec[t].add(Qtp1valsptr-Qtp1valsvec[t],1<<"
               << hw(Qtp1mask)
               << ");"
                  "\n		if (Qtp1valsptr-Qtp1valsvec[t]>0) loop_data_vec[t+20].add(Qtp1valsptr-Qtp1valsvec[t],1<<"
               << hw(Qtp1mask)
               << ");"
                  "\n		const uint32 checkmask_mask = "
               << Qtp1mask
               << ";"
            ...
            of << "\n		const uint32 Qtp1val = Qvalue[offset+t+1] ^ (Qprev[offset+t+1]&Q[offset+t]) ^ ((~metset1) & " << metmaskfix
               ...
               << ") ^ Qtp1val;"
                  "\n			m[t] = Q[offset+t+1] + mpartial;"
                  "\n			if (0 != ((m[t]^metset1)&"
               << metmasknofix << ")) " << (t >= 13 || t == 4 || t == 6 || t == 7 ? "break;" : "continue;")
        ...
        if (hw(metmaskinv & (~Qtp1maskinv)) <= MAX_LOOP_SIZE || (Qtp1maskinv & ~metmaskinv) == 0) {
            ...
            of << "\n		const uint32 Qtp1val = Qvalue[offset+t+1] ^ (Qprev[offset+t+1]&Q[offset+t]);"
                  "\n		const uint32 metset1adj = metset1 ^ ((~Qtp1val) & "
               << Qtp1maskfix << ") ^ (xrng64()&" << metmaskinv
               << ");"
                  "\n		const uint32 mpartial = mpartial2 - rotate_left(Q[offset+t],5);"
                  "\n		metpartialvec[t] = mpartial;"
                  "\n		uint32* Qtp1valsptr = Qtp1valsvec[t];"
               ...
               << "^curmask;"
                  ...
                  "\n			if (0 != ((Q[offset+t+1]^Qtp1val)&"
               << Qtp1masknofix
               << ")) break;"
                  "\n			uint32 Qtp1xorfix = (Q[offset+t+1]^Qtp1val) & "
               << Qtp1maskfix
               << ";"
               ...
               << hw(metmaskinv)
               << ");"
                  "\n		if (Qtp1valsptr-Qtp1valsvec[t]>0) loop_data_vec[20+t].add(Qtp1valsptr-Qtp1valsvec[t],1<<"
               << hw(metmaskinv)
               << ");"
                  "\n		const uint32 checkmask_mask = "
               << metmaskinv
            ...
            of << "\n		const uint32 Qtp1val = Qvalue[offset+t+1] ^ (Qprev[offset+t+1]&Q[offset+t]);"
               ...
               << ") ^ metset1adj;"
                  "\n			Q[offset+t+1] = m[t] - mpartial;"
                  "\n			if (0 != ((Q[offset+t+1]^Qtp1val)&"
               << Qtp1masknofix << ")) " << (t >= 13 || t == 4 || t == 6 || t == 7 ? "break;" : "continue;")
    ...
    if (t > 0) {
        for (unsigned i = t + 1; i < 16; ++i) {
            ...
            of << "\n		metset1precomp[" << t << "][" << i << "] = metset1precomp[" << t - 1 << "][" << i << "] ^ "
               << optimized_mecond_to_string("m[" + boost::lexical_cast<string>(t - 1) + "]", dependency_metm1) << ";";
        }
    }
    ...
    of << "\n	return step" << boost::lexical_cast<string>(t)
       << "ok;"
          "\n}";
    return of.str();
}
...
void generate_program() {
    ...
    of << program_header
       << "\n#include <hashclash/saveload.hpp>"
          "\n"
          "\n#define MAX_LOOP_SIZE "
       << MAX_LOOP_SIZE
       << "\n"
          "\nuint64 tendcount = 0;"
          "\nuint64 testcounts[1<<16];"
          "\nuint32 m_diff[80] = "
       << array_to_cppstring(mxordiff, 80)
       << ";"
          "\nconst uint32 dQ[85] = "
       << array_to_cppstring(dQ, 85)
       << ";"
          "\nconst uint32 Qvaluemask[85] = "
       << array_to_cppstring(Qvaluemask, 85)
       << ";"
          "\nconst uint32 Qvaluemask_adj[85] = "
       << array_to_cppstring(Qvaluemask_adj, 85)
       << ";"
          "\nconst uint32 Qvalue[85] = "
       << array_to_cppstring(Qvalue, 85)
       << ";"
          "\nconst uint32 Qprev[85] = "
       << array_to_cppstring(Qprev, 85)
       << ";"
          "\nconst uint32 Qprev2[85] = "
       << array_to_cppstring(Qprev2, 85)
       << ";"
          "\nconst uint32 dF[80] = "
       << array_to_cppstring(dF, 80)
       << ";"
          "\nvector< vector< vector<uint32> > > pathbitrelationsmatrix;"
          "\nchar pathbitrelationsmatrixstring[] = \""
       << pathbitrelationsmatrixstring
       << "\";"
       ...
       << "\nvoid step0();"
          ...
          "\n	{ stringstream tmp; tmp.str(string(pathbitrelationsmatrixstring)); boost::archive::text_iarchive ia(tmp); ia >> "
          "boost::serialization::make_nvp(\"pathbitrelationsmatrix\", pathbitrelationsmatrix); }"
          "\n	for (unsigned t = 0; t < 16; ++t)"
          "\n		for (unsigned b = 0; b < 32; ++b)"
    ...
    of << "\nvoid step_16()"
          ...
          "\n			Q2[offset+t+1] = m2[t] + sha1_f1(Q2[offset+t-1], rotate_left(Q2[offset+t-2],30), "
          "rotate_left(Q2[offset+t-3],30))"
          ...
          "\n			Q2[offset+t+1] = m2[t] + sha1_f2(Q2[offset+t-1], rotate_left(Q2[offset+t-2],30), "
          "rotate_left(Q2[offset+t-3],30))"
          ...
          "\n		}"
          "\n		if (t==33 && Q2[offset+33]==Q[offset+33] && Q2[offset+32]==Q[offset+32] && Q2[offset+31]==Q[offset+31] && "
          "Q2[offset+30]==Q[offset+30] && Q2[offset+29]==Q[offset+29]) {"
          "\n			cout << \"!\" << endl;"
          "\n			avg_data_vec[40].add_sum(1);"
          "\n			++time_avg[40];"
          "\n			for (int j = 15; j <= 33; ++j)"
          "\n				cout << \"dQ\" << j << \"=\" << sdr(Q[offset+j],Q2[offset+j]) << \"\\tdM\" << j << \"=\" << "
          "sdr(m[j],m2[j]) << endl;"
...
const char program_header[] = "\n#include <cmath>"
                              ...
                              "\n        uint32 a = Q[offset+tstart], b = Q[offset+tstart-1], c = Qr30[offset+tstart-2], d = "
                              "Qr30[offset+tstart-3], e = Qr30[offset+tstart-4];"
...
const char program_statistics[] =
    ...
    "\n			cout << \"t=\" << i << \"\t\" << round(double(cpu_step_t[i]-cpu_step_t[i+1])*1000.0/double(cpuspend)) << \"\t\" << "
    "cpu_step_t[i]-cpu_step_t[i+1] << endl;"
    "\n     for (unsigned i = 16; i < 80; ++i) {"
    "\n             if (cpu_step_t[i] > (cpuspend<<1))"
    "\n                     cout << \"i=\" << i << \"\t\" << round(double(cpu_step_t[i]+cputime)*1000.0/double(cpuspend)) << \"\t\" << "
    "(cpu_step_t[i]+cputime) << endl;"
    "\n             else if (cpu_step_t[i])"
    "\n                     cout << \"i=\" << i << \"\t\" << round(double(cpu_step_t[i])*1000.0/double(cpuspend)) << \"\t\" << "
    "(cpu_step_t[i]) << endl;"
    "\n     }"
    "\n	if (correct_cpu_step_t != -1) {"
    "\n		for (int t = 0; t <= correct_cpu_step_t; ++t)"
    ...
    "\n		if (time_avg[i])"
    "\n			cout << \"timeavg \" << i << \": \t2^\" << log(double(time_avg[i])/double(runtime_sw.time()))/log(2.0) << \"#/s\" "
    "<< endl;"
    ...
    "\n	for (unsigned t = 0; t < tend && t < 20; ++t) {"
    "\n		bool terror = false;"
    "\n		uint32 Qtp1 = m[t] + sha1_f1(Q[offset+t-1], rotate_left(Q[offset+t-2],30), rotate_left(Q[offset+t-3],30)) + sha1_ac[0] + "
    "rotate_left(Q[offset+t],5) + rotate_left(Q[offset+t-4],30);"
    ...
    "\n			cout << hex << ((Q[offset+t+1]^Qvalue[offset+t+1]^(Qprev[offset+t+1]&Q[offset+t])) & Qvaluemask[offset+t+1]) << "
    "dec << endl;"
    "\n			terror = true;"
    "\n		}"
    "\n		if (t < 16) {"
