...
#define AT_MIN_COUNT (AT_TEST_COUNT >> AT_MIN_PROB)
    ...
    for (unsigned k = 0; k < AT_TEST_COUNT; ++k) {
        ...
        while (t < 17) {
            ...
            for (unsigned j = 0; j < 3; ++j) {
                ...
                if (t >= 1) {
                    m[t - 1] = Q[offset + t] -
                               (sha1_f1(Q[offset + t - 2], rotate_left(Q[offset + t - 3], 30), rotate_left(Q[offset + t - 4], 30)) +
                                sha1_ac[0] + rotate_left(Q[offset + t - 1], 5) + rotate_left(Q[offset + t - 5], 30));
                    m2[t - 1] = Q2[offset + t] -
                                (sha1_f1(Q2[offset + t - 2], rotate_left(Q2[offset + t - 3], 30), rotate_left(Q2[offset + t - 4], 30)) +
                                 sha1_ac[0] + rotate_left(Q2[offset + t - 1], 5) + rotate_left(Q2[offset + t - 5], 30));
    ...
    int tret = 80;
    for (int t = 17; t < 27; ++t) {
        if (tcnts[t] >= AT_MIN_COUNT) {
            tret = t;
            break;
        }
        if (double(tcnts[t]) >= double(AT_TEST_COUNT) * 0.1) {
            tret = t;
            break;
        }
    }
...
#define STAT_ANA(ba, go, s)                                                                                                    \
    if (STAT_TEST(ba, go)) {                                                                                                   \
        cout << "poss. bc: q_" << t << "[" << b << "]=" << s << ": \t" << (double(100 * ba) / double(ba + go)) << "%" << endl; \
    }
void stat_ana(
    vector<vector<uint32>> &goodqtb,
    vector<vector<uint32>> &badqtb,
    vector<vector<uint32>> &goodq2tb,
    vector<vector<uint32>> &badq2tb,
    int t,
    int b
) {
    ...
    for (unsigned i = 0; i < goodqtb.size(); ++i) {
        ...
        if (GOODBIT(i, t, b) == GOODBIT(i, t - 1, b)) {
            ++g_prev;
        } else {
            ++g_prevn;
        }
        if (GOODBIT(i, t, b) == GOODBIT(i, t - 1, ((b + 2) & 31))) {
            ++g_prevr;
        } else {
            ++g_prevrn;
        }
        if (GOODBIT(i, t, b) == GOODBIT(i, t - 2, ((b + 2) & 31))) {
            ++g_prev2;
        } else {
            ++g_prev2n;
        }
    ...
    for (unsigned i = 0; i < badqtb.size(); ++i) {
        ...
        if (BADBIT(i, t, b) == BADBIT(i, t - 1, b)) {
            ++b_prev;
        } else {
            ++b_prevn;
        }
        if (BADBIT(i, t, b) == BADBIT(i, t - 1, ((b + 2) & 31))) {
            ++b_prevr;
        } else {
            ++b_prevrn;
        }
        if (BADBIT(i, t, b) == BADBIT(i, t - 2, ((b + 2) & 31))) {
            ++b_prev2;
        } else {
            ++b_prev2n;
        }
...
#define ATLC_MIN_COUNT (ATLC_TEST_COUNT >> ATLC_MIN_PROB)
    ...
    for (unsigned mi = 0; mi < mcnt; ++mi) {
        ...
        cout << goodqtb.size() << " vs " << badqtb.size() << ": "
             << (double(100) * double(badqtb.size())) / double(goodqtb.size() + badqtb.size()) << "% bad" << endl;
...
void find_best_tunnel(
    vector<sha1differentialpath> &tunnels, const sha1differentialpath &mypath, const vector<vector<uint32>> &bitrels, bool force_lc = false
) {
    ...
    for (unsigned i = 0; i < tunnels.size(); ++i) {
        ...
        for (int t = 0; t < 16; ++t) {
            sdrsvec[t].clear();
            if (t < tunnels[i].tbegin() || t >= tunnels[i].tend()) {
                continue;
            }
        ...
        for (uint64 c = 0; c < cnt; ++c) {
            ...
            if (!filter_tunnel_bitrelations(tunnels[i], bitrels)) {
                continue;
            }
            unsigned iQ;
            if (force_lc) {
...
void create_tunnel_stept(vector<sha1differentialpath> &tunnels, sha1differentialpath &tunnel, int t, bool force_lc = false) {
    ...
    for (unsigned b = 0; b < 32; ++b) {
        ...
        if (bo.size() == 0) {
            throw std::runtime_error("bo.size() == 0");
        }
        if (bo.size() == 1) {
            continue;
        }
        if (bo.size() == 2) {
            ...
            newconditions[b][0] =
                SHA1_F1_data.backwardconditions(tunnel(t - 1, b), tunnel(t - 2, (b + 2) & 31), tunnel(t - 3, (b + 2) & 31), bo[0]);
            newconditions[b][1] =
                SHA1_F1_data.backwardconditions(tunnel(t - 1, b), tunnel(t - 2, (b + 2) & 31), tunnel(t - 3, (b + 2) & 31), bo[1]);
            cnt *= 2;
            bitactive[b] = true;
        } else { // bo.size() == 3
            bf_conditions bf =
                SHA1_F1_data.backwardconditions(tunnel(t - 1, b), tunnel(t - 2, (b + 2) & 31), tunnel(t - 3, (b + 2) & 31), bc_constant);
#if 1
            ...
            if (bf.second != bc_prev) {
                throw std::runtime_error("bf.second != bc_prev");
            }
    ...
    for (unsigned c = 0; c < cnt; ++c) {
        ...
        uint32 dme =
            tunnel[t + 1].diff() - dFt - tunnel[t].getsdr().rotate_left(5).adddiff() - tunnel[t - 4].getsdr().rotate_left(30).adddiff();
        tunnel.getme(t) = naf(dme);
        create_tunnel_stept(tunnels, tunnel, t - 1, force_lc);
    }
}
...
void analyze_tunnels_diffpath(
    const sha1differentialpath &mypath, const vector<vector<uint32>> &bitrels, vector<sha1differentialpath> tunnelsin
) {
    ...
    for (int bit1 = 0; bit1 < 16 * 32; ++bit1) {
        for (int bit2 = bit1 + 1; bit2 < 16 * 32; ++bit2) {
            if (goodbits.find(bit1) != goodbits.end()) {
                continue;
            }
            if (goodbits.find(bit2) != goodbits.end()) {
                continue;
            }
            int t1 = bit1 / 32;
            int b1 = bit1 % 32;
            ...
            for (int bit2sign = 0; bit2sign < 1; ++bit2sign) {
                for (int bit3 = bit2; bit3 < 16 * 32; ++bit3) {
                    for (int bit3sign = 0; bit3sign < 1; ++bit3sign) {
                        ...
                        if (!mainparameters.threebittunnels && bit3 != bit2) {
                            continue; // force max 2 bit difference
                        }
                        if (bit1 == bit2 && bit2sign == 1) {
                            continue;
                        }
                        if (bit3 == bit2 && bit3sign != bit2sign) {
                            continue;
                        }
                        ...
                        if (goodbits.find(bit3) != goodbits.end()) {
                            continue;
                        }
                        if (good2bits.find(pair<int, int>(bit1, bit2)) != good2bits.end()) {
                            continue;
                        }
                        if (good2bits.find(pair<int, int>(bit1, bit3)) != good2bits.end()) {
                            continue;
                        }
                        if (good2bits.find(pair<int, int>(bit2, bit3)) != good2bits.end()) {
                            continue;
                        }
...
void analyze_indepsection_prob() {
    ...
    for (; tbegin < 80; ++tbegin) {
        if (0 ==
            (path[tbegin].diff() | path[tbegin - 1].diff() | path[tbegin - 2].diff() | path[tbegin - 3].diff() | path[tbegin - 4].diff())) {
            break;
        }
    }
    while (tbegin < 80) {
    ...
    while (i < dihvs.size()) {
        unsigned j = i + 1;
        while (j < dihvs.size() && dihvs[j] == dihvs[i]) {
            ++j;
        }
