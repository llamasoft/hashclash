...
void mespace_to_pathbitrelationsmatrix(const sha1messagespace &mespace, vector<vector<vector<uint32>>> &pathbitrelationsmatrix) {
    ...
    for (unsigned i = 0; i < bitrels.size(); ++i) {
        for (int col = (16 * 32) - 1; true; --col) {
            if (col < 0) {
                throw std::runtime_error("mespace_to_pathbitrelationsmatrix(): col < 0");
            }
            unsigned t = col >> 5;
            unsigned b = col & 31;
            if (bitrels[i][t] & (1 << b)) {
...
bool add_bitcondition(sha1differentialpath &path, int t, unsigned b, bitcondition bc) {
    ...
    if (bctb == bc_plus || bctb == bc_minus || bc == bc_plus || bc == bc_minus) {
        return false;
    }
    // since bctb!= bc these ifs test for a contradiction
    if ((bctb == bc_one || bctb == bc_zero) && (bc == bc_one || bc == bc_zero)) {
        return false;
    }
    if ((bctb == bc_prev || bctb == bc_prevn) && (bc == bc_prev || bc == bc_prevn)) {
        return false;
    }
    if ((bctb == bc_prev2 || bctb == bc_prev2n) && (bc == bc_prev2 || bc == bc_prev2n)) {
        return false;
    }
    if ((bctb == bc_next || bctb == bc_nextn) && (bc == bc_next || bc == bc_nextn)) {
        return false;
    }
    if ((bctb == bc_next2 || bctb == bc_next2n) && (bc == bc_prev2 || bc == bc_next2n)) {
        return false;
    }
