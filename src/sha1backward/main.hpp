...
void sha1_backward_differential_step(const hashclash::sha1differentialpath &path, path_container_autobalance &container);
  ...
  public:
    path_container_autobalance()
        : modn(1),
          ...
          minweight(0),
          fillfraction(0),
          onemessagediff(false),
          expandprevmessagediff(false) {
        for (unsigned k = 0; k < 80; ++k) {
            m_mask[k] = 0;
        }
    }

    ~path_container_autobalance() { cerr << "Autobalance parameters: maxcond=" << maxcond << endl; }
    ...
    void push_back(const sha1differentialpath &path, unsigned cond = 0) {
        if (cond == 0) {
            for (int k = int(t) - 3; k <= tend && k <= 80; ++k) {
                if (k < path.tend()) {
                    cond += path[k].hw();
                }
            }
            if (includenaf) {
                if (halfnafweight) {
                    cond += (path[int(t) - 4].hw() >> 1);
                } else {
                    cond += path[int(t) - 4].hw();
                }
            }
        }
        if (cond > maxcond) {
        ...
        if (maxQ26upcond > 0) {
            unsigned cond2 = 0;
            for (int k = int(t) - 3; k < path.tend() && k <= 80; ++k) {
                if (k >= 26) {
                    cond2 += path[k].hw();
                }
            }
