...
void sha1_backward_differential_step(const sha1differentialpath &path, path_container_autobalance &outpaths) {
    ...
    booleanfunction *F = 0;
    if (t < 20) {
        F = &SHA1_F1_data; // this implementation only works properly for sha1_f1
    } else if (t < 40) {
        F = &SHA1_F2_data;
    } else if (t < 60) {
        F = &SHA1_F3_data;
    ...
    if (totcond + Qtm3_hwnaf + minextracond > maxcond) {
        return;
    }
    ...
    while ((w < 32) && (w + 1 <= maxweight) && (totcond + w + 1 + minextracond <= maxcond) &&
            ...
            for (unsigned j = 0; j < bval.size(); ++j) {
                ...
                if (b < 31) {
                    ...
                    if (msb_bf_outcome(*F, newconditions).size() > 1 || msb_bf_outcome(*F, newconditions)[0] != foutcomes[b][i]) {
                        cout << endl
                             << "[" << Qtm1b[b] << Qtm2b[b] << Qtm3b[b] << "](" << foutcomes[b][i] << ")=>[" << newconditions.first
                             << newconditions.second << newconditions.third << "]" << endl;
