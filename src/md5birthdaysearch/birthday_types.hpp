...
// CUDA compiler fails when using non-trivial constructors on device memory
    ...
    bool operator==(const trail_type &rhs) const {
        return start[0] == rhs.start[0] && start[1] == rhs.start[1] && start[2] == rhs.start[2] && end[0] == rhs.end[0] &&
               end[1] == rhs.end[1] && end[2] == rhs.end[2] && len == rhs.len;
    }
    bool operator!=(const trail_type &rhs) const { return !(*this == rhs); }
};
...
class cuda_device {
  public:
    bool init(
        ...
        const uint32 ihv1[4],
        const uint32 ihv2[4],
        const uint32 ihv2mod[4],
        const uint32 msg1[16],
        const uint32 msg2[16],
        uint32 hmask,
        uint32 dpmask,
        uint32 maxlen
    );
    void cuda_fill_trail_buffer(
        uint32 id, uint64 seed, vector<trail_type> &buffer, vector<pair<trail_type, trail_type>> &collisions, bool mod = false
    );
    void benchmark();
...
class simd_device_avx256 {
  public:
    bool init(
        ...
        const uint32 ihv2[4],
        const uint32 ihv2mod[4],
        const uint32 precomp1[4],
        const uint32 precomp2[4],
        const uint32 msg1[16],
        const uint32 msg2[16],
        uint32 hmask,
        uint32 dpmask,
