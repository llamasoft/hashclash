/*****
  Copyright (C) 2015 Pierre Karpman, INRIA France/Nanyang Technological University Singapore (-2016), CWI (2016/2017), L'Universite Grenoble
Alpes (2017-) (C) 2015 Marc Stevens, Centrum Wiskunde & Informatica (CWI), Amsterdam.
...
// template<size_t N, typename val_t = uint32_t, size_t val_cnt = 1, typename control_type = cyclic_buffer_control_mask_t<N>, int fencetype
// = 2 > class cyclic_buffer_mask_t;
...
// template<size_t N, typename val_t = uint32_t, size_t val_cnt = 1, typename control_type = cyclic_buffer_control_cas_t<N>, int fencetype =
// 2 > class cyclic_buffer_cas_t;
...
/* UTILITY TYPES */
template <bool b> struct assert_compile_time_bool {};
template <> struct assert_compile_time_bool<true> {
...
template <size_t i, bool pred = true> struct pred_t {};
...
template <size_t N> class cyclic_buffer_control_mask_t {
  public:
    ...
    __host__ __device__ inline uint32_t used_count() { return write_idx - read_idx; }
    ...
    template <typename safereadval_t> __device__ inline uint32_t warp_read_idx(safereadval_t safereadvals[N]) {
        while (true) {
    ...
    // assumes entire warp calls this function with identical inputs
    // if read is safe return per-thread idx to read from, returns 0xFFFFFFFF otherwise
    template <typename safereadval_t> __device__ inline uint32_t warp_read_idx(uint32_t warp_to_read_mask, safereadval_t safereadvals[N]) {
        // warp: determine count and offset
    ...
    // assumes entire warp calls this function with identical inputs
    // if there is at least 1 element and read is safe return per-thread idx to read from or 0xEEEEEEEE if there was no input for that
    // thread, returns 0xFFFFFFFF otherwise (no work at all)
    template <typename safereadval_t> __device__ inline uint32_t warp_read_any_idx(safereadval_t safereadvals[N]) {
        while (true) {
            // obtain warp-wide unique race-free value of read_idx
            uint32_t baseidx = __shfl_sync(WARP_FULL_MASK, read_idx, 0);
            uint32_t thrdidx = (baseidx + (threadIdx.x & 31)) % N;
    ...
    template <typename safereadval_t> __host__ inline uint32_t host_read_idx(safereadval_t safereadvals[N]) {
...
template <
    size_t N,
    typename val_t = uint32_t,
    size_t val_cnt = 1,
    typename control_type = cyclic_buffer_control_mask_t<N>,
    int fencetype = 2>
class cyclic_buffer_mask_t {
  public:
    ...
    template <size_t i, bool pred> __host__ __device__ inline void cond_write2(uint32_t j, const val_t &v, pred_t<i, pred>) {
        val[i][j % N] = v;
    }
    template <size_t i> __host__ __device__ inline void cond_write2(uint32_t j, const val_t &v, pred_t<i, false>) {}
    template <size_t i, bool pred> __host__ __device__ inline void cond_write(uint32_t j, const val_t &v, pred_t<i, pred>) {
    ...
    __host__ __device__ inline void write(
        control_t &control,
        bool dowrite,
        val_t _val0,
        val_t _val1 = 0,
    ...
    template <size_t i> __host__ __device__ inline val_t get(uint32_t idx) {
        ASSERT_COMPILE_TIME(i < val_cnt);
        return val[i][idx % N];
    }
    template <size_t i> __host__ __device__ inline val_t &get_ref(uint32_t idx) {
        ASSERT_COMPILE_TIME(i < val_cnt);
        return val[i][idx % N];
    }
};
...
template <size_t N> class cyclic_buffer_control_cas_t {
  public:
    ...
    // variant partial warp read
    // assumes entire warp calls this function with identical inputs
    // if read is safe return per-thread idx to read from, 0xEEEEEEEE for no thread-specific input, or returns 0xFFFFFFFF otherwise (no work
    // at all)
...
template <
    ...
    typename val_t = uint32_t,
    size_t val_cnt = 1,
    typename control_type = cyclic_buffer_control_cas_t<N>,
    int fencetype = 2>
...
class warp_tmp_buf_t {
  public:
    ...
    template <typename buffer_t, typename control_t> __device__ inline void flush1(buffer_t &buf, control_t &ctrl) {
    ...
    template <typename buffer_t, typename control_t> __device__ inline void flush2(buffer_t &buf, control_t &ctrl) {
        uint32_t baseidx = (uint32_t)(idx) & 32;
        if (idx != baseidx) {
