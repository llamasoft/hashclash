...
typedef cyclic_buffer_mask_t<MAX_CUDA_THREADS_PER_BLOCK, uint32, 7, cyclic_buffer_control_mask_t<MAX_CUDA_THREADS_PER_BLOCK>, 1>
    work_buffer_t;
...
bool cuda_device::init(
    uint32 device,
    const uint32 ihv1b[4],
    const uint32 ihv2b[4],
    const uint32 ihv2modb[4],
    const uint32 msg1b[16],
    const uint32 msg2b[16],
    uint32 hmask,
    uint32 dpmask,
    uint32 maxlen
) {
    ...
    cout << "Using " << detail->blocks << " blocks with " << detail->threadsperblock << " threads each: total "
         << detail->blocks * detail->threadsperblock << " threads." << endl;
...
template <bool mod = false> __device__ void cuda_md5_work2(uint64 seed) {
    halt_flag = 0;
    /********************* GENERATE TRAILS ***********************/
    restore_controls();
...
template <bool mod = false> __global__ void cuda_md5_work(uint64 seed) { cuda_md5_work2<mod>(seed); }

template <bool mod = false> __global__ void cuda_md5_collisions(uint64 seed) {
    halt_flag = 0;
...
void cuda_device::cuda_fill_trail_buffer(
    uint32 id, uint64 seed, vector<trail_type> &buf, vector<pair<trail_type, trail_type>> &collisions, bool mod
) {
    //	CUDA_SAFE_CALL( cudaMallocHost( (void**)(&(detail->trailsout_buf)), detail->blocks * sizeof(work_buffer_t) ) );
    //	CUDA_SAFE_CALL( cudaMallocHost( (void**)(&(detail->trailsout_ctl)), detail->blocks * sizeof(work_control_t) ) );
    ...
    if (detail->collisions.size()) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            detail->collisionsin_buf->write(
                *(detail->collisionsin_ctl),
                true,
                detail->collisions[i].first.start[0],
                detail->collisions[i].first.start[1],
                detail->collisions[i].first.start[2],
                detail->collisions[i].first.end[0],
                detail->collisions[i].first.end[1],
                detail->collisions[i].first.end[2],
                detail->collisions[i].first.len,
                detail->collisions[i].second.start[0],
                detail->collisions[i].second.start[1],
                detail->collisions[i].second.start[2],
                detail->collisions[i].second.end[0],
                detail->collisions[i].second.end[1],
                detail->collisions[i].second.end[2],
        ...
        if (0) {
            std::cout << "C: " << oldsize << " " << detail->collisions.size() << " " << detail->collisionsin_ctl->used_count() << " "
                      << detail->collisionsin_ctl->free_count() << " " << collisionblocks << " " << detail->blocks << std::endl;
        }
    }
    ...
    */
    ...
    if (collisionblocks > 0) {
        ...
        uint32 readidx;
        while ((readidx = detail->collisionsout_buf->getreadidx(*(detail->collisionsout_ctl))) < 0xEEEEEEEE) {
...
timer::~timer() { delete detail; }

timer::timer(bool direct_start)
    : running(false) {
