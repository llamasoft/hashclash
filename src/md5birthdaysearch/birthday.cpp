...
void status_line() {
    ...
    cout << ", Coll.: " << main_storage.get_totcoll() << "(uf=" << collusefull << ",nuf=" << collequalihvs
         << ",?=" << (totcoll - collusefull - collequalihvs - collrobinhoods - collqueue) << ",q=" << collqueue << ",rh=" << collrobinhoods
...
void find_collision(const trail_type &trail1, const trail_type &trail2) {
    ...
    */
    ...
    uint32 dihv[4] = {ihvmsg2[0] - ihvmsg1[0], ihvmsg2[1] - ihvmsg1[1], ihvmsg2[2] - ihvmsg1[2], ihvmsg2[3] - ihvmsg1[3]};
    ...
    if (maxblocks == 1) {
        ...
        } else if (dihv[0] == (1 << 5) && dihv[3] == (1 << 5) - (1 << 25) &&
                   (dihv[2] & hybridmask) == (uint32((1 << 5) - (1 << 25) - (1 << 23)) & hybridmask)) {
            pair<uint32, uint32> bc(-dihv[1], -dihv[2]);
            if (binary_search(singleblockdata.begin(), singleblockdata.end(), bc)) {
                n = 1;
...
void load_save_trails(bool dosave = true) {
    ...
    try {
        ...
        for (; dit != ditend; ++dit) {
            ...
            if (!exists(*dit) || is_symlink(*dit) || is_directory(*dit)) {
                continue;
            }
        ...
        try {
            ...
            for (unsigned i = 0; i < procmodn; ++i) {
                ...
                if (traildata.trails.size()) {
                    try {
                        ++fileserialothers[i];
                        string basefilename = workdir + "/" + boost::lexical_cast<string>(i) + "/birthdaydata_" +
                                              boost::lexical_cast<string>(procmodi) + "_" + boost::lexical_cast<string>(xrng128()) + "_";
                        string tmpfilename = "/tmp/birthdaydata_" + boost::lexical_cast<string>(i) + "_" +
                                             boost::lexical_cast<string>(procmodi) + "_" + boost::lexical_cast<string>(xrng128()) + "_";
                        save(traildata, binary_archive, tmpfilename + boost::lexical_cast<string>(fileserialothers[i]) + ".tmp");
                        boost::filesystem::copy_file(
                            tmpfilename + boost::lexical_cast<string>(fileserialothers[i]) + ".tmp",
...
struct birthday_thread {
        ...
        while (true) {
            uint64 seed;
            {
                ...
                /*
                                                ...
                                                {
                                                        if (!haveenoughcoll)
                                                                std::cout << "Thread " << id << " (CUDA): started processing colliding
                   trails on GPU" << std::endl; main_storage.get_birthdaycollisions(collisions); haveenoughcoll = true;
                                                }
                */
                if (quit) {
                    return;
            ...
            if (collisions.size() > 0) {
                //				std::cout << "Thread " << id << " (CUDA): " << collisions.size() << " colliding trails
                //returned" << std::endl;
        ...
        try {
            {
                ...
                if (_cuda_device_nr >= 0) {
                    ...
                    if (!_cuda_device.init(
                            _cuda_device_nr, ihv1, ihv2, ihv2mod, msg1, msg2, hybridmask, distinguishedpointmask, maximumpathlength
                        )) {
                        return;
                    }
...
#endif // CUDA
                    if (_simd_device.init(
                            ihv1, ihv2, ihv2mod, precomp1, precomp2, msg1, msg2, hybridmask, distinguishedpointmask, maximumpathlength
                        )) {
    ...
    void single_run() { loop(true); }

    void operator()() { thread_run(); }
};
uint32 birthday_thread::id_counter = 1;
struct birthday_thread_shell {
    birthday_thread *bt;
    birthday_thread_shell(birthday_thread *_bt)
        : bt(_bt) {}
...
void birthday(birthday_parameters &parameters) {
    ...
    cout << "Maximum amount of memory in MB for trails: " << parameters.maxmemory << " (local: " << parameters.maxmemory / parameters.modn
         << ")" << endl;
    cout << "Estimated number of trails that will be stored:    " << maxtrails << " (local: " << maxtrails / parameters.modn << ")" << endl;
    cout << "Estimated number of trails that will be generated: " << uint64(pow(double(2), estcomplexity - parameters.logpathlength))
         << endl;
