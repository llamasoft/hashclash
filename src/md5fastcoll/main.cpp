...
void find_collision(
    const uint32 IV[], uint32 msg1block0[], uint32 msg1block1[], uint32 msg2block0[], uint32 msg2block1[], bool verbose = false
);
...
int main(int argc, char **argv) {
    ...
    cout << "MD5 collision generator v1.5\n"
    ...
    try {
        ...
        desc.add_options()("help,h", "Show options.")("quiet,q", "Be less verbose.")("ihv,i", po::value<string>(&ihv), "Use specified initial value. Default is MD5 initial value.")("seed1", po::value<uint32_t>(&seed32_1), "Specify a SEED1 value.  Default is based on time.")("seed2", po::value<uint32_t>(&seed32_2), "Specify a SEED2 value.  Default is a constant value.")("prefixfile,p", po::value<string>(&prefixfn), "Calculate initial value using given prefixfile. Also copies data to output files.")(
            "out,o",
            po::value<vector<string>>()->multitoken(),
            "Set output filenames. This must be the last option and exactly 2 filenames must be specified. \nDefault: -o msg1.bin msg2.bin"
        );

        po::options_description hidden;
        hidden
            .add_options()("testmd5iv", "Endlessly time collision generation using MD5 initial value.")("testrndiv", "Endlessly time collision generation using arbitrary random initial values.")("testreciv", "Endlessly time collision generation using recommended random initial values.")(
        ...
        if (vm.count("prefixfile")) {
            ...
            if (l >= 4 && prefixfn[l - 4] == '.' && prefixfn[l - 3] != '.' && prefixfn[l - 2] != '.' && prefixfn[l - 1] != '.') {
                ...
                unsigned i = 1;
                while (fs::exists(fs::path(outfn1)) || fs::exists(fs::path(outfn2))) {
