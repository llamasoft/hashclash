...
void sha1_forward_differential_step(const hashclash::sha1differentialpath &path, path_container_autobalance &container);
            ...
            if (k >= path.tbegin() && k < path.tend()) {
                for (unsigned b = 0; b < 32; ++b) {
                    ...
                    case bc_prevn: {
                        ...
                        if (bcpath == bc_plus || bcpath == bc_minus || bcpathm1 == bc_plus || bcpathm1 == bc_minus) {
                            return;
                        }
                        if (bcpathm1 == bc_constant) {
                            break;
