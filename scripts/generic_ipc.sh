...
###################### FIRST NEAR-COLLISION BLOCK PAIR #####################
	...
	if [[ ! -s data/bestpath.bin.gz ]]; then (( datalower *= 2 )); echo "First step failed. Restarting"; sleep 2; continue; fi
...
################## SECOND NEAR-COLLISION BLOCK PAIR #########################
	...
	if [[ ! -s data/bestpath.bin.gz ]]; then (( datalower *= 2 )); echo "Second step failed. Restarting"; sleep 2; continue; fi
